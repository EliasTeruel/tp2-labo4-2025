...
import { FormatoHoraPipe } from '../../pipes/fotmato-hora.pipe';
import { FotrmatoFechaPipe } from '../../pipes/fotmato-fecha.pipe';
@Component({
  selector: 'app-turnos',
  standalone: true,
  imports: [CommonModule, FormsModule,
    CapitalizarPipe,
    FormatoHoraPipe,FotrmatoFechaPipe],
...
export class TurnosComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.rol === 'admin') {
    this.pacientes = await this.supabaseService.getPacientes();
  }
  ...
  const disponibles = ['dentista', 'clinico', 'traumatologo', 'medico'];
  return disponibles.includes(normalizado) ? `${normalizado}.png` : 'default.png';
}

seleccionarEspecialidad(especialidad: any) {
  this.especialidadSeleccionada = especialidad.id;
  this.onEspecialidadChange();
}


seleccionarEspecialista(prof: any) {
  this.especialistaSeleccionado = prof.id;
  this.onEspecialistaChange();
}
reemplazarConImagenLocal(event: Event) {
  const imgElement = event.target as HTMLImageElement;
  imgElement.src = '/default.jpg';
}

async cargarEspecialistas(especialidadId: string) {
  this.especialistas = await this.supabaseService.getEspecialistasPorEspecialidad(especialidadId);
}
